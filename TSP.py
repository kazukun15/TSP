...
def solve_tsp(dist_mat):
    """1秒以内の Guided Local Search + 2-opt"""
    n=len(dist_mat)
    mgr=pywrapcp.RoutingIndexManager(n,1,0)
    routing=pywrapcp.RoutingModel(mgr)
    def cb(f,t):
        return int(dist_mat[mgr.IndexToNode(f),mgr.IndexToNode(t)]*1e5)
    ...
    # 2-opt で後処理
    route_opt = two_opt(route, dist_mat)
    return route_opt
...
with st.sidebar.form("manual"):
    ...
    if st.form_submit_button("追加"):
        st.session_state.shelters=pd.concat(
            [st.session_state.shelters,pd.DataFrame([{"lat":lat,"lon":lon,"name":name}])],
            ignore_index=True
        )
    ...
    if len(sel)<2:
        ...
        if not route:
            ...
            for k in range(len(route)-1):
                ...
                if G and ni is not None and nj is not None:
                    try:
                        seg=nx.shortest_path(G,ni,nj,weight="length")
                ...
                if not path: path.extend(coords)
                else: path.extend(coords[1:])
...
st.pydeck_chart(
    pdk.Deck(map_style=map_styles[map_style],initial_view_state=view,layers=layers,
